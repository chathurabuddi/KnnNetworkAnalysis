...
public class KnnNetworkAnalysis {
    ...
    private static Instance determineFraudStatus(
        ...
        for(Neighbor neighbor : nearestNeighbors) {
            if(neighbor.getInstance().isFraud()) frudCount++;
            else normalCount++;
        }

        if(frudCount > normalCount) classificationInstance.setFraudStatus(true);
        else classificationInstance.setFraudStatus(false);
